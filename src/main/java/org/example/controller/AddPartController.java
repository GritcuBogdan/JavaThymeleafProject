...
public class AddPartController {
    ...
    public String deletePart(@Valid @RequestParam("partID") long theId,  Model theModel){
        PartService repo = context.getBean(PartServiceImpl.class);
        Part part=repo.findById(Math.toIntExact(theId));
        if(part.getProducts().isEmpty()){
            repo.deleteById(Math.toIntExact(theId));
